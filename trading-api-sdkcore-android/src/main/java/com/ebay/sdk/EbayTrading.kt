...
class EbayTrading(
    ...
    val tokenLifecycleCallback: TokenLifecycleCallback? = null) {
    ...
    companion object {
        ...
        // ponytail: matches eBayUtil.DATE_FORMAT / TIME_ZONE from the desktop SDK
        private val API_TIME_FORMATTER: DateTimeFormatter =
    ...
    private fun notifyHardExpirationWarning(resp: AbstractResponseType) {
        val warning = resp.hardExpirationWarning ?: return
        if (warning.isEmpty()) return
        tokenLifecycleCallback?.onHardExpirationWarning(parseApiTimeString(warning))
    }

    private fun parseApiTimeString(strDate: String): Instant =
        API_TIME_FORMATTER.parse(strDate, Instant::from)
    ...
    fun call(callName: String, request: AbstractRequestType, accessTokenSupplier: () -> String) : AbstractResponseType {
        suspend fun inner() : AbstractResponseType {
            ...
            val resp = unmarshal(response.body.bytes(), wsMethod.returnType) as AbstractResponseType
            notifyHardExpirationWarning(resp)
            return resp
        }
